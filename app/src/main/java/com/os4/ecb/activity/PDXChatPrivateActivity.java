...
public class PDXChatPrivateActivity extends BaseActivity {

	private static final String TAG = PDXChatPrivateActivity.class.getName();
	IPDXServiceExt pdxService = null;

	private EditText msgText;
	private ListView msgListView;
	private DiscoItems.Item item;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		item = (new Gson()).fromJson(json, DiscoItems.Item.class);

		String name = item.getJid().substring(item.getJid().lastIndexOf("/")+1);
		getSupportActionBar().setTitle(name.substring(0,1).toUpperCase().concat(name.substring(1)));
		getSupportActionBar().setSubtitle(item.getJid());
	...
	public class MessageReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			try {
				String json = (String) pdxService.getMessagesParticipant(item.getJid());
	...
	protected void onPause() {
		...
		try{
			if(pdxService!=null)
				pdxService.setCurrentParticipant(null);
		}catch(Exception e){
        ...
        public void onServiceConnected(ComponentName className, IBinder service) {
            ...
            try{
				if(pdxService!=null) {
					...
					pdxService.setCurrentParticipant(item.getJid());
					String json = (String) pdxService.getMessagesParticipant(item.getJid());
					messages = (new Gson()).fromJson(json, Messages.class);
					if(messages!=null) {
	...
	public void processMessage(String msg){
		try{
			...
			if(pdxService!=null) pdxService.sendMessageParticipant(item.getJid(),msg,null);
		}catch(Exception e){e.printStackTrace();}
	}
	...
	private class MessagesAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup viewgroup) {
			...
			LayoutInflater mInflater = PDXChatPrivateActivity.this.getLayoutInflater();
	...
	private View.OnClickListener onImageDownloadListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			try{
				...
				if(pdxService!=null) pdxService.acceptTransferFile(msg.getId(),item.getJid());
